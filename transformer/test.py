...
def run_encoder(encoder_model, texts, return_weights=False):
    ...
    with torch.no_grad(): # Disable gradient calculations
        output, weights = encoder_model(texts, return_last_layer_attn_weights=return_weights)
...
def test_forward_pass_shape():
    ...
    assert encoder_output.shape[0] == len(test_texts), f"Expected batch size {len(test_texts)}, got {encoder_output.shape[0]}"
    assert encoder_output.shape[1] == expected_seq_len, f"Expected seq len {expected_seq_len}, got {encoder_output.shape[1]}"
    assert encoder_output.shape[2] == TEST_CONFIG["d_model"], f"Expected d_model {TEST_CONFIG['d_model']}, got {encoder_output.shape[2]}"
...
def test_batch_independence():
    ...
    encoder_output, _ = run_encoder(encoder, test_texts) # run_encoder handles eval() and no_grad()
    ...
    assert are_identical, "Test 2 Failed: Outputs for identical inputs in a batch differ!"
    print("Test 2 Passed: Model produces deterministic outputs for identical inputs in a batch (in eval mode).")
...
def test_padding_mask_effectiveness():
    ...
    # Get the tokenizer object first, then call it
    tokenizer_obj = encoder.tokenizer
    encodings = tokenizer_obj(test_texts, padding='longest', return_tensors='pt').to(device)
    # --- END CORRECTION ---
    ...
    if not non_padding_indices_seq0:
        print("Test 3 Warning: Sequence 0 consists only of padding/special tokens. Cannot test attention.")
        return
    if not padding_indices_seq0:
        print("Test 3 Skipped: No padding occurred in sequence 0 for this batch.")
        return

    first_non_padding_idx_seq0 = non_padding_indices_seq0[0] # Index of first real token
    print(f"Test 3: First non-padding index in sequence 0: {first_non_padding_idx_seq0}")
    ...
    assert last_attn_weights is not None, "Test 3 Failed: Did not receive attention weights."

    # Check attention weights for the FIRST non-padding token in the FIRST sequence
    query_token_idx = first_non_padding_idx_seq0
    attn_weights_for_token = last_attn_weights[0, :, query_token_idx, :] # (num_heads, seq_len_k)

    # Get the weights specifically for the padded key positions
    weights_on_padding = attn_weights_for_token[:, padding_indices_seq0] # (num_heads, num_padding_tokens)
    ...
    max_weight_on_padding = torch.max(weights_on_padding).item() if weights_on_padding.numel() > 0 else 0.0
    print(f"Test 3: Max attention weight on padding tokens (for query token {query_token_idx}): {max_weight_on_padding:.2E}") # Scientific notation

    padding_ignored = max_weight_on_padding < 1e-6
    assert padding_ignored, f"Test 3 Failed: Significant attention weight ({max_weight_on_padding:.2E}) found on padding tokens!"
...
def test_positional_encoding_effect():
    ...
    if encoder_output.shape[1] > 1: # If sequence length > 1
        ...
        print(f"Test 4: Output for token 0 and token 1 in the same sequence are different: {are_different}")
        assert are_different, "Test 4 Failed: Output for adjacent tokens is identical. Check PE or attention."
